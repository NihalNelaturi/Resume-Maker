          ...
          {rewrites.map((rewrite) => (
            ...
            >
              ...
              <div className="grid gap-3 lg:grid-cols-2">
                <TextBlock label="Before" text={rewrite.original} />
                <TextBlock label="After" text={rewrite.rewritten} />
              </div>
...
function TextBlock({ label, text }) {
  return (
    <div>
      ...
      <p className="mt-1 rounded-md border border-slate-200 bg-white p-3 text-sm leading-6 text-slate-700">{text}</p>
    </div>
  );
}
